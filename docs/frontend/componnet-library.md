...
最简单的方式就是单独成一个仓库，再嵌入到各业务项目中，达到一处更新，多个项目使用的效果。
组件的预览及调试通常还是和业务结合。
...
#### 方式四、文档系统
开源项目中有像vitepress, dumi这样可以解析markdown及vue\react组件的系统。整合此类系统后，可以达到可视化预览组件、书写组件的props等信息。
...
### 组件的技术设计
#### 业务场景的抽象
通常每个组件都是多个业务场景的聚合。像表格、表单这样的组件，更是涉及到系统的所有列表及表单页面。

所以做技术设计时需要考虑多种场景，而不是像业务开发时的专一场景。

不同的场景会直接影响到组件的props、事件及插槽等设计。

#### 组件的层级带来通信的复杂度
为了增强复用性，组件库拆分的颗粒度都会比较细。这是为了方便后续的维护及复用。
这样也会带来新的问题，组件的层级会比较深。特别是有页面级配置组件时。数据的传递层级会比较深。
在开发时，通信的复杂度也会增加。
...
5. play系统：替换成公司内部的base模板，并增加基于包及组件库代码调试的切换开关（这里的play系统类似ant-design-pro，是对组件库及业务场景结合运用时最佳实践的阐释）
6. 组件管理工具的增强：通过一定的命令行提示，初始化相关的组件，及删除组件的功能
